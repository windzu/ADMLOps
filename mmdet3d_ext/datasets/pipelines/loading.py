...
class LoadPointsFromPointCloud2:
    """Load Points From LoadPointsFromPointCloud2. 加载式 point cloud 2 格式的点云数据.
    ...
    def __init__(
        ...
        assert max(
            use_dim
        ) < load_dim, f'Expect all used dimensions < {load_dim}, got {use_dim}'
    ...
    def _pc_format_converter(self, input_data):
        ...
        因为实例化模型是根据配置文件来的，所以转换后的点云要与配置文件中一致，否则会导致维度出错。 例如：
        ...
        Args:
            input_data (PointCloud2): 输入的点云数据,来自ros1订阅,为ros1 sensor_msgs 的
        PointCloud2类型的点云数据
        """
...
@PIPELINES.register_module()
class LoadPointsFromFileExtension:
    """修改自 LoadPointsFromFile , 解决加载数据维度问题 nuscenes数据集的原始点云是[x, y, z,
    intensity,ring] 5维,但是一般数据集只有4维度, 这里采用自己扩增fake数据来解决这个问题,
    ...
    def __call__(self, results):
        ...
        if self.dim_diff_flag:
            points = np.concatenate(
                [points, np.zeros((points.shape[0], self.dim_diff))], axis=1)

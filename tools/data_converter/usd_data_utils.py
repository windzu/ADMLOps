...
def __label_postprocess(label):
    ...
    def annos_postprocess(annos):
        ...
        annos['bbox2d'] = np.array(
            annos['bbox2d'], dtype=np.int32).reshape(-1, 4)

        # bbox3d原始标注中仅有7个值,但是为了支持mmdetetion3d中的pipeline需要转换9个值,因此尾部补全两个0
        annos['bbox3d'] = np.array(
            annos['bbox3d'], dtype=np.float32).reshape(-1, 7)
        annos['bbox3d'] = np.concatenate(
            [annos['bbox3d'],
             np.zeros((annos['bbox3d'].shape[0], 2))], axis=1)

        annos['truncated'] = np.array(
            annos['truncated'], dtype=np.int32).reshape(-1)
        annos['occluded'] = np.array(
            annos['occluded'], dtype=np.int32).reshape(-1)
        if annos['num_points_in_gt'] is None:
            annos['num_points_in_gt'] = np.zeros(
                len(annos['class_names'], dtype=np.int32))
        else:
            annos['num_points_in_gt'] = np.array(
                annos['num_points_in_gt']).reshape(-1)
        return annos
    ...
    def calib_postprocess(calib):
        ...
        if 'intrinsics' in calib:
            for key in calib['intrinsics']:
                calib['intrinsics'][key] = np.array(
                    calib['intrinsics'][key]).reshape(3, 3)
        for key in calib:
            if key != 'intrinsics':

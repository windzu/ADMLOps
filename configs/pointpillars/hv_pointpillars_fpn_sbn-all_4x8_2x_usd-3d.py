...
"""
data_root = os.path.join(os.environ['ADMLOPS'], 'data', 'USD_Apollo')
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=os.path.join(data_root, 'usd_infos_train.pkl'),
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=os.path.join(data_root, 'usd_infos_val.pkl'),
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(type='DeltaXYZWLHRBBoxCoder',
                        code_size=9),  # 3dbbox默认编码为7，这里改为9
        loss_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,

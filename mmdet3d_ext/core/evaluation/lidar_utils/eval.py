...
# 定义评估时各个类别的iou阈值
# 但是由于不同类别的size不一样，如果采用同一个iou阈值会导致某些类别的某些框被评估为0,
# 例如: 在 overlap 为0.7时候 car的 overlap可以设置为0.7 但是person的 overlap可以设置为0.5(不然person类别的框会很容易被评估为0)
# nus default
...
def lidar_eval(gt_annos, dt_annos, current_classes, eval_types=["bev", "3d"]):
    """lidar数据集的eval方法
    修改自kitti的 evaluation. kitti中支持2dbbox 3dbbox_bev 3d 三种评估方式,本方法仅支持 bev 和 3d

    NOTE : 因为评估方式不同写起来很费事,所以首先将lidar格式的标注数据转换为kitti格式的标注数据,
    ...
    def convert_lidar_annos_to_kitti_annos(lidar_annos):
        """将lidar格式的标注数据转换为kitti格式的标注数据
        两者的区别主要在于:
            1. lidar格式的标注数据中location dimensions rotation_y都是在lidar坐标系下的,
        ...
        for anno in lidar_annos:
            ...
            # truncated occluded alpha bbox use 0 as dummy score
